...
const BulkStaffCustomerMap = () => {
  const [apiData, setApiData] = useState("");
  const [pageCount, setPageCount] = useState(1);
  const [loading, setLoading] = useState(false);
  const context = useContext(Context);
  const { setModuleData, setBulkUploadCSVOpen } = context;
  ...
  useEffect(() => {
    dispatch(bulkUploadingService("ASSIGN-STAFF-CUSTOMERS", pageCount));
    setLoading(false);
  }, [pageCount]);
    ...
    <div>
      <h2 className="page_title">
        ...
        <div className="breadcrumb">
          ...
          <span>/ Bulk-Uploading-Staff-Customer-Mapping</span>
        </div>
      </h2>
      <Content
        style={{
      ...
      >
        <div style={styleHeader}>
          <Button
            type="primary"
            onClick={() => {
              setModuleData("ASSIGN-STAFF-CUSTOMERS");
...
export default BulkStaffCustomerMap;
